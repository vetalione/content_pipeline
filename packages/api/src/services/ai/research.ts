import OpenAI from 'openai';
import { prisma } from '../../lib/db';
import { PipelineStage, ResearchData, BiographyFact } from '@content-pipeline/shared';

const openai = new OpenAI({
  apiKey: process.env.OPENAI_API_KEY
});

/**
 * Perform research on a celebrity
 */
export async function performResearch(articleId: string): Promise<ResearchData> {
  const article = await prisma.article.findUnique({
    where: { id: articleId }
  });
  
  if (!article) {
    throw new Error('Article not found');
  }
  
  console.log(`Researching ${article.celebrityName}...`);
  
  // Create research prompt
  const prompt = createResearchPrompt(article.celebrityName);
  
  // Call OpenAI with timeout
  console.log('Calling OpenAI API for research...');
  
  const completion = await Promise.race([
    openai.chat.completions.create({
      model: 'gpt-4-turbo-preview',
      messages: [
        {
          role: 'system',
          content: `Ты эксперт-исследователь, специализирующийся на поиске КОНКРЕТНЫХ драматических фактов о знаменитостях для статей в стиле "Great Losers" (канал на Дзен).

ЦЕЛЬ ИССЛЕДОВАНИЯ:
Собрать материал для статьи формата "[N] неудач [ИМЯ]". Читатель знает героя как успешную личность, но мы покажем скрытый путь через провалы.

КРИТИЧЕСКИ ВАЖНО - ищи ТОЛЬКО:
✅ Конкретные неудачи с цифрами, датами, суммами
✅ Драматические низшие точки карьеры (ОСОБЕННО в детстве/юности!)
✅ Скандалы, провалы, банкротства, зависимости
✅ Тюрьма, бедность, издевательства, травмы
✅ Отказы, увольнения, критика
✅ Контрасты: "был никем → стал звездой"

ОБЯЗАТЕЛЬНО указывай:
- Точный ВОЗРАСТ в момент события (для хронологического порядка!)
- Конкретные суммы денег
- Сколько дней/месяцев/лет длилось
- Имена людей, мест, компаний
- Цитаты героя (если есть)

ОСОБОЕ ВНИМАНИЕ:
🎯 **ДЕТСТВО/ЮНОСТЬ** - ищи редкие детские фото, факты из школы, первой работы
🎯 **ХРОНОЛОГИЯ** - от детства к текущему моменту (возраст обязателен!)
🎯 **ТИЗЕР** - факт для первого абзаца: "чем известен + малоизвестная интрига"

❌ НЕ НУЖНЫ общие фразы типа "столкнулся с трудностями"
✅ НУЖНЫ детали типа "потерял $500,000", "провёл 113 дней в тюрьме", "в 14 лет работал разносчиком газет"

Формат ответа: минимум 8-10 драматических фактов-неудач В ХРОНОЛОГИЧЕСКОМ ПОРЯДКЕ + итоговый успех с цифрами`
        },
        {
          role: 'user',
          content: prompt
        }
      ],
      temperature: 0.7,
      max_tokens: 6000,
      response_format: { type: 'json_object' }
    }),
    new Promise((_, reject) => 
      setTimeout(() => reject(new Error('OpenAI API timeout after 3 minutes')), 180000)
    )
  ]) as OpenAI.Chat.Completions.ChatCompletion;
  
  console.log('OpenAI response received, parsing JSON...');
  
  const researchData = JSON.parse(completion.choices[0].message.content || '{}');
  
  // Save to database
  await prisma.article.update({
    where: { id: articleId },
    data: {
      researchData,
      currentStage: PipelineStage.RESEARCH,
      updatedAt: new Date()
    }
  });
  
  return researchData;
}

function createResearchPrompt(celebrityName: string): string {
  return `
Исследуй жизнь ${celebrityName} и найди минимум 8-10 КОНКРЕТНЫХ драматических неудач/провалов.

ОБЯЗАТЕЛЬНЫЙ JSON формат:

{
  "teaser": {
    "known_for": "Чем знаменит и любим всем миром (2-3 факта)",
    "hidden_drama": "Малоизвестные драматические повороты судьбы (интрига для первого абзаца)",
    "childhood_photo_hint": "Описание редкого детского фото, которое нужно найти"
  },
  "failures": [
    {
      "number": 1,
      "title": "Краткий цепляющий заголовок с деталью",
      "age": "возраст когда это случилось (ОБЯЗАТЕЛЬНО для хронологии!)",
      "year": "год события",
      "description": "Детальное описание со всеми цифрами, суммами, сроками",
      "outcome": "что из этого вышло",
      "severity": 1-5,
      "visual_suggestion": "Какое фото/гифку/мем можно сделать для этого пункта"
    }
  ],
  "quotes": [
    {
      "text": "Точная цитата героя",
      "context": "Когда и почему он это сказал",
      "source": "Откуда цитата",
      "suitable_for_ending": true/false
    }
  ],
  "success": {
    "peak_achievement": "Главное достижение с цифрами",
    "current_status": "Текущий статус",
    "wealth": "Состояние/гонорары с суммами",
    "awards": ["список наград"],
    "personal_life": "Семья, дети (если есть)"
  },
  "bonus_fact": "Забавный или шокирующий малоизвестный факт",
  "timeline": "Краткая хронология: от [худшее] в [возраст/год] до [лучшее] в [возраст/год]",
  "sources": ["источник1", "источник2"]
}

КРИТИЧНО - каждая неудача должна содержать:
- Точный возраст или год (для выстраивания ХРОНОЛОГИИ от детства к взрослому)
- Конкретные цифры (деньги, дни, километры, что угодно)
- Драматические детали с иронией
- Никаких общих фраз!

ПОРЯДОК НЕУДАЧ - СТРОГО ХРОНОЛОГИЧЕСКИЙ:
- Начинай с детства (5-7 лет)
- Затем школа/юность (10-16 лет)
- Молодость и первая работа (17-25 лет)
- Зрелость и карьера (25+ лет)

Примеры ХОРОШИХ заголовков:
✅ "В 5 лет пришлось заменять маму на сцене" (Чаплин)
✅ "С 14 лет работал уборщиком" (Керри)
✅ "Провёл 113 дней в настоящей тюрьме" (Дауни)
✅ "В 30 лет подметал улицы, рыл канавы" (Тесла)

Примеры ПЛОХИХ заголовков:
❌ "Испытал финансовые трудности"
❌ "Столкнулся с проблемами"
❌ "Пережил сложный период"

Фокусируйся на:
- Детство: бедность, приюты, развод родителей, школьные издевательства
- Зависимости, банкротства, тюрьма
- Отказы, увольнения, провальные проекты
- Травмы, болезни, потери близких
- Скандалы, судебные дела
- Бедность, долги, бездомность

История должна иметь хэппи-энд с конкретными цифрами успеха! И финальная цитата героя о преодолении.
`;
}

